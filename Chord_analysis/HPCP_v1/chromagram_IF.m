...
fminl = octs2hz(hz2octs(f_ctr, sr)-2*f_sd, sr);
fminu = octs2hz(hz2octs(f_ctr, sr)-f_sd, sr);
fmaxl = octs2hz(hz2octs(f_ctr, sr)+f_sd, sr);
fmaxu = octs2hz(hz2octs(f_ctr, sr)+2*f_sd, sr);
...
function hz = octs2hz (octs,fT)
    hz = fT/16.0*power(2,octs);
end
   
function octs = hz2octs(freq,fT)
